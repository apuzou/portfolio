...
@section('page-title', 'プロジェクト編集')

@section('content')
    <div class="max-w-4xl">
        <div class="bg-white shadow-sm rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h1 class="text-lg font-medium text-gray-900">プロジェクトを編集</h1>
            </div>
            ...
            <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" required
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-300 @enderror">{{ old('description', $project->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="6"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('content') border-red-300 @enderror">{{ old('content', $project->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="draft" {{ old('status', $project->status) === 'draft' ? 'selected' : '' }}>下書き
                        </option>
                ...
                <div>
                    ...
                    <input type="text" id="technologies" name="technologies_input"
                        value="{{ old('technologies_input', $project->technologies->pluck('name')->join(', ')) }}"
                        placeholder="例: PHP, Laravel, MySQL, JavaScript, Vue.js"
