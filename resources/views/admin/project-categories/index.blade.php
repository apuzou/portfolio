...
@section('title', 'プロジェクトカテゴリ管理')
@section('page-title', 'プロジェクトカテゴリ管理')

@section('content')
    <div class="space-y-6">
        <!-- ヘッダー -->
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold text-gray-900">プロジェクトカテゴリ一覧</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">プロジェクト数
                        </th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($categories as $category)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $category->projects->count() }}件
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                <a href="{{ route('admin.project-categories.show', $category) }}"
                                ...
                                <form method="POST" action="{{ route('admin.project-categories.destroy', $category) }}"
                                    class="inline" onsubmit="return confirm('このカテゴリを削除しますか？関連するプロジェクトも削除されます。')">
                                    @csrf
                                    @method('DELETE')
